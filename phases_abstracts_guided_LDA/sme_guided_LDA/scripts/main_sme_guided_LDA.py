...
from sme_common_words import seed_words
from tm_sme_guided_LDA import perform_topic_modeling_on_excel_data_with_guided_lda, save_topics_and_coherence_to_file
import pyLDAvis
...
def perform_topic_modeling_from_excel(num_topics, num_words):
    ...
    # Step 3: Load seed words
    seed_words_file_path = os.getenv('COMMON_TERMS_FILE_PATH')
    try:
        with open(seed_words_file_path, 'r') as f:
            ...
            if total_seed_words < num_topics:
                print(f"Error: You have {total_seed_words} seed words but requested {num_topics} topics.")
                return

            print(f"Distributing {total_seed_words} seed words across {num_topics} topics...")
        ...
        print(f"Error: The file {seed_words_file_path} does not exist.")
        return
