...
def visualize_all(topic_model, documents, topics, output_dir, num_topics, num_keywords_per_topic):
    ...
    try:
        ...
        fig.write_html(os.path.join(output_dir, "topics_umap_custom_preprocessing_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topics_umap_custom_preprocessing_solitude.png"))
    except Exception as e:
        print(f"UMAP visualization error: {e}")

    try:
        fig = topic_model.visualize_hierarchy(top_n_topics=num_topics)
        fig.write_html(os.path.join(output_dir, "topic_hierarchy_custom_preprocessing_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topic_hierarchy_custom_preprocessing_solitude.png"))
    except Exception as e:
        print(f"Hierarchy visualization error: {e}")

    try:
        fig = topic_model.visualize_barchart(top_n_topics=10)
        fig.write_html(os.path.join(output_dir, "topic_barchart_custom_preprocessing_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topic_barchart_custom_preprocessing_solitude.png"))
    except Exception as e:
        print(f"Barchart error: {e}")

    try:
        fig = topic_model.visualize_heatmap(top_n_topics=num_topics)
        fig.write_html(os.path.join(output_dir, "topic_heatmap_custom_preprocessing_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topic_heatmap_custom_preprocessing_solitude.png"))
    except Exception as e:
...
def save_results(documents, topics, probs, topic_model, topic_labels, output_dir, num_keywords_per_topic, summaries=None):
    ...
    """
    txt_path = os.path.join(output_dir, "bertopic_results_custom_preprocessing_solitude.txt")
    csv_path = os.path.join(output_dir, "bertopic_results_custom_preprocessing_solitude.csv")
    excel_path = os.path.join(output_dir, "bertopic_topic_summary_custom_preprocessing_solitude.xlsx")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("BERTopic custom preprocessing pipeline solitude results\n\n")
        for topic_id in topic_model.get_topic_info()['Topic']:
            if topic_id == -1:
                continue
            keywords = topic_model.get_topic(topic_id)
...
if __name__ == "__main__":
    ...
    results_folder = os.getenv("RESULTS_DIRECTORY_CUSTOM_PREPROCESSING_SOLITUDE", os.path.dirname(docx_file_path))
