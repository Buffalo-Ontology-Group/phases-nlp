...
def generate_specific_labels(topic_model, summaries, kw_model, num_keywords=3):
    ...
    for topic_id in topic_model.get_topic_info()['Topic']:
        ...
        label = f"Solitude – {label_keyword}"
...
def visualize_all(topic_model, documents, topics, output_dir, num_topics, num_keywords_per_topic):
    ...
    try:
        ...
        fig.write_html(os.path.join(output_dir, "topics_umap_default_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topics_umap_default_solitude.png"))
    except Exception as e:
        print(f"UMAP visualization error: {e}")

    try:
        fig = topic_model.visualize_hierarchy(top_n_topics=num_topics)
        fig.write_html(os.path.join(output_dir, "topic_hierarchy_default_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topic_hierarchy_default_solitude.png"))
    except Exception as e:
        print(f"Hierarchy visualization error: {e}")

    try:
        fig = topic_model.visualize_barchart(top_n_topics=10)
        fig.write_html(os.path.join(output_dir, "topic_barchart_default_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topic_barchart_default_solitude.png"))
    except Exception as e:
        print(f"Barchart error: {e}")

    try:
        fig = topic_model.visualize_heatmap(top_n_topics=num_topics)
        fig.write_html(os.path.join(output_dir, "topic_heatmap_default_solitude.html"))
        fig.write_image(os.path.join(output_dir, "topic_heatmap_default_solitude.png"))
    except Exception as e:
...
def save_results(documents, topics, probs, topic_model, topic_labels, output_dir, num_keywords_per_topic, summaries=None):
    ...
    Rules (applied to ALL outputs):
      - Auto_Label := topic_labels[topic_id]
      - Descriptive_Label := "" if Auto_Label unique
                             "{Auto_Label} – {TopKeyword}" if Auto_Label repeated
    """
    import os
    from collections import Counter
    import pandas as pd

    txt_path = os.path.join(output_dir, "bertopic_results_default_solitude.txt")
    csv_path = os.path.join(output_dir, "bertopic_results_default_solitude.csv")
    excel_path = os.path.join(output_dir, "bertopic_topic_summary_default_solitude.xlsx")

    # Count auto-label frequencies
    label_counts = Counter(topic_labels.values())

    # Build duplicate-aware descriptive labels
    adjusted_descriptive_labels = {}
    for topic_id, auto_label in topic_labels.items():
        if topic_id == -1:
            adjusted_descriptive_labels[topic_id] = "Noise / Outlier"
            continue
        ...
        if label_counts[auto_label] > 1:
            ...
            descriptive_label = ""  # blank when unique
        adjusted_descriptive_labels[topic_id] = descriptive_label

    # ---------- TXT (topic-level + doc-level) ----------
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("BERTopic default pipeline solitude results\n\n")
        ...
        for topic_id in topic_model.get_topic_info()['Topic']:
            ...
            label_desc = adjusted_descriptive_labels.get(topic_id, "")

            f.write(f"Topic {topic_id}\n")
            f.write(f"Auto Label: {label_auto}\n")
            if label_desc:
                f.write(f"Descriptive Label: {label_desc}\n")
        ...
        for i, prob in enumerate(probs):
            ...
            label_desc = adjusted_descriptive_labels.get(assigned_topic, "")
            label_text = label_desc if label_desc else label_auto
            try:
                prob_float = float(prob)
            except Exception:
                prob_float = prob
            f.write(
                f"Doc {i + 1}: Topic {assigned_topic} ({label_text}), "
                f"Auto: {label_auto}, Prob: {prob_float:.8f}\n"
            )

    # ---------- CSV (document-level; same labels) ----------
    rows = []
    for i, prob in enumerate(probs):
        assigned_topic = topics[i]
        label_auto = topic_labels.get(assigned_topic, f"Topic {assigned_topic}")
        ...
        rows.append({
            ...
            "Descriptive_Label": label_desc,
            "Probability": prob_float
        })
    pd.DataFrame(rows).to_csv(csv_path, index=False)

    # ---------- Excel (topic-level; same labels) ----------
    topic_data = []
    for topic_id in topic_model.get_topic_info()['Topic']:
        if topic_id == -1:
            continue
...
if __name__ == "__main__":
    ...
    results_folder = os.getenv("RESULTS_DIRECTORY_DEFAULT_SOLITUDE", os.path.dirname(docx_file_path))
