...
def parse_bertopic_txt(file_path):
    """
    Parses a BERTopic-generated TXT file to extract the full label inside parentheses
    (e.g., "Aloneness – Solitude") along with keywords and summary.
    """
    topics = {}
    current_topic = None
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Match lines like: Topic 3 (Aloneness – Solitude)
            if line.startswith("Topic "):
                match = re.match(r"Topic\s+(\d+)\s*\(([^)]+)\)", line)
                current_topic = int(line.split()[1])
            ...
            elif line.startswith("Keywords:") and current_topic is not None:
                topics[current_topic]["keywords"] = line.replace("Keywords:", "").strip()

            elif line.startswith("Summary:") and current_topic is not None:
...
merged_df["merged_topic"] = merged_df.apply(
    lambda r: f"S{r['solitude_id']}: {r['solitude_label']} → G{r['gerotranscendence_id']}: {r['gerotranscendence_label']}",
    axis=1
)
...
node_trace = go.Scatter(
    x=node_x, y=node_y, mode='markers+text', text=node_text, textposition="top center",  textfont=dict(size=20, color="black"),
...
# --- Adjust layout for readable long labels ---
fig.update_layout(
    title=dict(
        text='Merged Topic Label Graph with OpenIE Relations',
        font=dict(size=18)
    ),
    showlegend=False,
    hovermode='closest',

    # ✅ Key: widen margins and fix clipping
    margin=dict(l=400, r=150, t=80, b=60),

    # ✅ Prevent autoshrink and extend horizontal range
    autosize=False,
    width=1900,
    height=1000,
    ...
    xaxis=dict(
        ...
        zeroline=False,
        visible=False,
        range=[-0.5, 1.5]  # expands the visible area left/right
    ),
    yaxis=dict(
        showgrid=False,
        zeroline=False,
...
# ✅ Move text slightly right so it stays visible
node_trace.textposition = "middle right"
